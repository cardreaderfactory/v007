...
bool           flushed;
uint8_t        expanded_key_buffer[176];
uint8_t        buffer       [AES_BLOCKSIZE];
uint8_t        chainBlock   [AES_BLOCKSIZE];
uint8_t        writeCursor;
flash uint8_t  initialChain[] = {0x7F, 0x83, 0x71, 0x0D, 0x4F, 0x1F, 0x21, 0x71, 0x2a, 0xDC, 0x4A, 0x7f, 0x74, 0x33, 0x75, 0xC8};
...
;; should be able to avoid saving R0, R1, R22, R23, R24, R25, R26, R27, R30 and R31

.macro save_registers
    push r22       ; save r22 to stack      
    in r22, SREG   ; load contents of SREG into S22
    cli
...
//    pop r31
    out SREG, r22
    pop r22    ; the original contents of r22
.endm
...
;;; address byte equal to $00. In assembler syntax: low(sbox<<1) == 0.
;;; To ensure the proper alignment of the sboxes, the assembler directive
;;; .ORG is used (below the sboxes are defined to begin at $200). Note, that
;;; any other address can be used as well, as long as the lower byte is equal
;;; to $00.
;;;
;;; The order of the sboxes is totally arbitrary. They even do not have to be
...
;;; ***************************************************************************
    ...
    save_registers ; save registers into stack

    ; ---------------- expand encrypt key ----------------

    ldi ZH, high(_buffer)   ; load key into ST11-ST44
    ldi ZL, low(_buffer)
    ld ST11, Z+
    ld ST21, Z+
    ld ST31, Z+
    ...
    ldi   R23,high(_expanded_key_buffer)
    ldi   R22,low(_expanded_key_buffer)
    mov YH,r23      ; expand key to the memory
    mov YL,r22      ; location is provided in *key_buffer - last parametter
    rcall key_expand

    restore_registers ; restore registers from the stack

    LD   R17,Y+     ; don't know why I need to do this ... code copied from all the functions generated by cvavr.
;;    ADIW R28,2      ; in CVAVR we have to add the number of bytes received as parametters
    ret
...
#asm
    ...
    ldd   r27,y+1   ; msb raw buffer
    ldd   r26,y+0   ; lsb raw buffer
    ldd   r25,y+3   ; msb crypted buffer
    ldd   r24,y+2   ; lsb crypted buffer
    ldi   R22,LOW(_expanded_key_buffer)
    ldi   R23,HIGH(_expanded_key_buffer)
    ...
    ldi YL, $b0  ; initialize YH:YL to point BEHIND $b0 = 176
    ldi YH, $00
    add YL, r22  ; decryption key material and
    adc YH, r23
    rcall decrypt       ; call decryption routine

    ; now the registers ST11-ST44 contain the plaintext again
    ; ------------ store - just for checking. delete after proven ok -----------

    mov ZH,r27 ; store the text into raw text buffer
    mov ZL,r26
    st Z+, ST11
    st Z+, ST21
    st Z+, ST31
    ...
    st Z+, ST44
    ; --------------- end decode -------------------------
    restore_registers   ; restore registers from stack
    ADIW R28,4          ; in CVAVR we have to add the number of bytes received as parametters
    ret
...
{
#asm
    save_registers   ; save registers to stack

    ldd   r27,y+3   ; msb raw buffer
    ldd   r26,y+2   ; lsb raw buffer
    ldd   r25,y+1   ; msb crypted buffer
    ldd   r24,y+0   ; lsb crypted buffer
    ldi   r22,LOW(_expanded_key_buffer)
    ldi   r23,HIGH(_expanded_key_buffer)
    ...
    mov YH,r23      ; initialize YH:YL to
    mov YL,r22      ; expanded key and call
    rcall encrypt   ; encryption routine

    ; now the registers ST11-ST44 contain the enciphered text

    ; -------------------- stode encoded data -----------------------
    mov ZH,r25 ; store the text into encrypted buffer
    mov ZL,r24
    st Z+, ST11
    st Z+, ST21
    st Z+, ST31
    ...
    restore_registers   ; restore registry from stack
    ADIW R28,4          ; in CVAVR we have to add the number of bytes received as parametters
    ret
...
void initChainBlock()
{
    ...
    {
        uint8_t r = (uint8_t)df_currentByte % AES_BLOCKSIZE;     
        if (r != 0)
        {
            //printf("changed df_currentByte from %lu to ", df_currentByte);
            df_currentByte += (AES_BLOCKSIZE - r);
            //printf("%lu\n", df_currentByte);        
        }
    }
#endif

    if (df_currentByte < AES_BLOCKSIZE)
    {
        for (i = 0; i < AES_BLOCKSIZE; i++)
            chainBlock[i] = initialChain[i];
    }
    else
    {
        df_enableContinuousRead(df_currentByte - AES_BLOCKSIZE);
        for (i = 0; i < AES_BLOCKSIZE; i++)
            chainBlock[i] = rwSpi(0x00);
        df_inactive();
    }
}

void bufferEncrypt(uint8_t *buf)
{
    uint8_t i;

    if(cypherMode != CBC)
    {
        aes_encrypt(buffer, buffer);
    }
    else
    {
        for (i = 0; i < AES_BLOCKSIZE; i++)
            buffer[i] ^= chainBlock[i]; /* xor bytes (buffer with chainBlock) */

        aes_encrypt(buffer, buffer);

        memcpy(chainBlock, buffer, AES_BLOCKSIZE);
    }

}

/* returns 0 if write fails (no memory left)
 * returns length if writes succeeds
 */
uint8_t aes_write( const uint8_t *buf, uint8_t length )
{
    ...
    toWrite = writeCursor + length;

//    printf("index = %i, len = %i, toWrite = %i\n", aes_writeCursor, length, toWrite);

    do
    {
        if (toWrite < AES_BLOCKSIZE)
        {
            memcpy(buffer + writeCursor, buf, toWrite - writeCursor);
            writeCursor = toWrite;
            toWrite = 0;
        }
        else
        {
            memcpy(buffer + writeCursor, buf, AES_BLOCKSIZE - writeCursor);
            toWrite -= AES_BLOCKSIZE;
            buf += AES_BLOCKSIZE - writeCursor;
            writeCursor = 0;

            bufferEncrypt(buffer);
            if ( df_write(buffer, AES_BLOCKSIZE) != AES_BLOCKSIZE )
            {
                length = 0;
                break;
            }
        }
//        printf("end loop: cursor = %i, total = %i\n", aes_writeCursor, toWrite);
    }
    while (toWrite != 0);

    flushed = false;

//    printf("end func: cursor = %i, toWrite = %i\n", aes_writeCursor, toWrite);
...
//    printf("\nAES_Flush: flushing\n");
    if (writeCursor != 0)
    {
        memset(buffer + writeCursor, ' ', AES_BLOCKSIZE - writeCursor);
        bufferEncrypt(buffer);
        df_write(buffer, AES_BLOCKSIZE);//write
    }
